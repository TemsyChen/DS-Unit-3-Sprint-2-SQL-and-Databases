...
The Employee and Territory tables have a "one to many" relationship, where one employee can be assigned to many territories. In reality, probably each employee is assigned more than one territory for sales or service, but a territory does not have more than one employee associated with it.
- What is a situation where a document store (like MongoDB) is appropriate, and what is a situation where it is not appropriate?
MongoDB is a NoSQL, document database, which means it is flexible and can be easily scaled to handle big data. MongoDB would be appropriate for a startup where they are still experimenting on their database structure, or may experience sudden growth. It would not be appropriate for a database that required security and highly structured organization, such as a bank. 
- What is "NewSQL", and what is it trying to achieve?
NewSQL attempts to have all the benefits of NoSQL and SQL relational databases. It is ACID compliant, which means it is has atomicity (functions are executed completely or not at all), is consistent (does not accept invalid data), isolated (executes functions in sequence to prevent "mid air collisions"), and durable (data is saved even when the system fails), and also has the ability to scale up easily to levels of big data.
